    ...
    public class FixedAssetGroupRepository : IFixedAssetGroupRepository
    {
        ...
        public FixedAssetGroup Add(FixedAssetGroup FAG, string userid)
        {
            var sqlQuery = @"INSERT INTO FIXEDASSETGROUP (IDFAGROUP, FAGROUPNAME, ESTIMATELIFEYEAR, COSTCENTER, INPUTBY, INPUTDATE, UPDATEBY, UPDATEDATE) VALUES
                           ('" + FAG.IDFAGROUP + @"', '" + FAG.FAGROUPNAME + "', '" + FAG.ESTIMATELIFEYEAR + "', '" + FAG.COSTCENTER + @"', " + Convert.ToInt32(userid) + ", '" + DateTime.Now + @"', " + Convert.ToInt32(userid) + ", '" + DateTime.Now + @"'); " + "SELECT CAST(SCOPE_IDENTITY() as int)";
        ...
        public FixedAssetGroup Update(FixedAssetGroup FAG, string userid)
        {
            var sqlQuery =
                ...
                FAG.IDFAGROUP, FAG.FAGROUPNAME, FAG.ESTIMATELIFEYEAR, FAG.COSTCENTER, Convert.ToInt32(userid), DateTime.Now, FAG.SID);
