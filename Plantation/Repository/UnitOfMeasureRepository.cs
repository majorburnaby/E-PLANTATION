...
namespace Plantation.Repository
{
    public class UnitOfMeasureRepository : IUnitOfMeasureRepository
    {
        ...
        public ROLEMENU GetRole(int IDMENU, string userid)
        {
            string query = string.Format(@"SELECT c.ISCANREAD, c.ISCANADD, c.ISCANEDIT, c.ISCANDELETE
                            FROM USERPROFILE a
                            LEFT JOIN USERROLE b
                            ON a.SID = b.IDUSER
                            LEFT JOIN ROLEMENU c
                            ON b.IDROLE = c.IDROLE
                            WHERE a.SID = '{0}' and c.IDMENU = {1}", userid, IDMENU);

            return this._db.Query<ROLEMENU>(query).SingleOrDefault();
        }
    }
}