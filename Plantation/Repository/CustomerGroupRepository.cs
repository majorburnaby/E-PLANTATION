...
namespace Plantation.Repository
{
    public class CustomerGroupRepository : ICustomerGroupRepository
    {
        ...
        {
            var sqlQuery = @"INSERT INTO CUSTOMERGROUP (IDCUSTOMERGROUP, CUSTOMERGROUPNAME, INPUTBY, INPUTDATE, UPDATEBY, UPDATEDATE) VALUES
                           ('" + CSG.IDCUSTOMERGROUP + @"', '" + CSG.CUSTOMERGROUPNAME + "', " + Convert.ToInt32(userid) + ", '" + DateTime.Now + @"', " + Convert.ToInt32(userid) + ", '" + DateTime.Now + @"'); " + "SELECT CAST(SCOPE_IDENTITY() as int)";
        ...
        {
            var sqlQuery =
                string.Format(@"UPDATE CUSTOMERGROUP SET IDCUSTOMERGROUP = '{0}', CUSTOMERGROUPNAME = '{1}', UPDATEBY = {2}, UPDATEDATE = '{3}' WHERE SID = {4}",
                CSG.IDCUSTOMERGROUP, CSG.CUSTOMERGROUPNAME, Convert.ToInt32(userid), DateTime.Now, CSG.SID);
