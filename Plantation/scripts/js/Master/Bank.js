                ...
                $row.children('td').each(function (i) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 8) {
                            $this.html('<select id="ddlCurrencyMaster" class="form-control input-sm"></select>');
                            $.getJSON('GetCurrencyMasterList', function (json) {
                                $('#ddlCurrencyMaster').empty();
                                $('#ddlCurrencyMaster').append($('<option>').text("Select"));
                                $.each(json, function (i, obj) {
                                    var selected = (data[9] == obj.SID) ? 'selected' : '';
                                    $('#ddlCurrencyMaster').append($('<option ' + selected + '>').text(obj.CURRENCYNAME).attr('value', obj.SID));
                                });
                            });
                        }

                        if ($(this).parent().children().index($(this)) === 12) {
                            ...
                            $.getJSON('GetCountryList', function (json) {
                                ...
                                $.each(json, function (i, obj) {
                                    var selected = (data[13] == obj.SID) ? 'selected' : '';
                                    $('#ddlCountry').append($('<option ' + selected + '>').text(obj.COUNTRYNAME).attr('value', obj.SID));
                                });
                            });
                        }

                        if ($(this).parent().children().index($(this)) === 14) {
                        ...
                        if ($(this).parent().children().index($(this)) === 16) {
                            ...
                            $.getJSON('GetCityList', function (json) {
                                ...
                                $.each(json, function (i, obj) {
                                    var selected = (data[17] == obj.SID) ? 'selected' : '';
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) == 8) {
                            $CurrencyName = $('#ddlCurrencyMaster').find(":selected").text();
                            return $('#ddlCurrencyMaster').find(":selected").val();
                        }
                        if ($(this).parent().children().index($(this)) == 12) {
                ...
                if (values[0]) {
                    bank.SID = values[0];
                    bank.IDBANK = values[1];
                    bank.BANKNAME = values[2];
                    bank.BANKACCOUNTCODE = values[3];
                    bank.ADDRESS = values[4];
                    bank.PHONE = values[5];
                    bank.FAX = values[6];
                    bank.DATEREGISTERED = values[7];
                    bank.CURRENCY = values[8];
                    bank.INACTIVE = values[10];
                    bank.INACTIVEDATE = values[11];
                    bank.COUNTRY = values[12];
                    bank.PROVINCE = values[14];
                    bank.CITY = values[16];
                    bank.POSCODE = values[18];
                    $.ajax({
                        type: "POST",
                        url: "/Bank/Edit",
