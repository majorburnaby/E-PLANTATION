        ...
        var Table = {
            ...
            build: function () {
                this.datatable = this.$table.DataTable({
                    //dom: "Bfrtip",
                    ajax: "/FixedAssetGroup/Data",
                    columns: [
                        { data: "FIXEDASSETGROUP.SID" },
                        { data: "FIXEDASSETGROUP.IDFAGROUP" },
                        { data: "FIXEDASSETGROUP.FAGROUPNAME" },
                        { data: "FIXEDASSETGROUP.ESTIMATELIFEYEAR" },
                        { data: "COSTCENTER.COSTCENTERNAME", editField: "FIXEDASSETGROUP.COSTCENTER" }
                    ...
                    "columnDefs": [
                        {
                            ...
                            "targets": 5, "data": "SID", "render": function (data, type, full, meta) {
                                return '<div class="SID" style="display:none;">' + data.FIXEDASSETGROUP.SID + '</div><a href="#" type="button" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row" data-toggle="tooltip" data-original-title="Edit"><i class="icon md-edit" aria-hidden="true"></i></a>'
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "FIXEDASSETGROUP": { "SID": 0, "IDFAGROUP": "", "FAGROUPNAME": "", "ESTIMATELIFEYEAR": "","COSTCENTER": "" }, "COSTCENTER": { "COSTCENTERNAME": "" } });
            ...
            rowEdit: function ($row) {
                ...
                $row.children('td').each(function (i) {
                    ...
                    if (arrdata.length < 3) {
                        if (i === 0) {
                            content = arrdata[0].IDFAGROUP;
                        }
                        else if (i === 1) {
                            content = arrdata[0].FAGROUPNAME;
                        }
                        else if (i === 2) {
                            content = arrdata[0].ESTIMATELIFEYEAR;
                        }
                        else if (i === 3) {
                        ...
                        if (i === 0) {
                            content = arrdata[1].IDFAGROUP;
                        }
                        else if (i === 1) {
                            content = arrdata[1].FAGROUPNAME;
                        }
                        else if (i === 2) {
                            content = arrdata[1].ESTIMATELIFEYEAR;
                        }
                        else if (i === 3) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            ...
                            $.getJSON('GetCostCenter', function (json) {
                                $('#ddlCostCenter').empty();
            ...
            rowSave: function ($row) {
                ...
                if (values[0] === "" || values[1] === "" || values[2] === "" || values[3] === "") {
                    ...
                    var fixedassetgroup = {};
                    if (values[4].FIXEDASSETGROUP.SID !== 0) {

                        fixedassetgroup.SID = values[4].FIXEDASSETGROUP.SID;
                        fixedassetgroup.IDFAGROUP = values[0];
                        fixedassetgroup.FAGROUPNAME = values[1];
                        fixedassetgroup.ESTIMATELIFEYEAR = values[2];
                        fixedassetgroup.COSTCENTER = values[3];
                        $.ajax({
                            type: "POST",
                            url: "/FixedAssetGroup/Edit",
                            data: '{fixedassetgroup: ' + JSON.stringify(fixedassetgroup) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
                    } else {
                    ...
                    $('#tblfixedassetgroup').DataTable().ajax.reload();
                }
            },
