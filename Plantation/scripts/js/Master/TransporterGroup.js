        ...
        var Table = {
            ...
            build: function () {
                this.datatable = this.$table.DataTable({
                    //dom: "Blfrtip",
                    ajax: "/TransporterGroup/Data",
                    columns: [
                        { data: "SID" },
                        { data: "IDTRANSPORTERGROUP" },
                        { data: "TRANSPORTERGROUPNAME" }
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "SID": 0, "IDTRANSPORTERGROUP": "", "TRANSPORTERGROUPNAME": "" });
            ...
            rowSave: function ($row) {
                ...
                if (values[0] === "" || values[1] === "") { //Empty Data Validation
                    ...
                    values[2].IDTRANSPORTERGROUP = values[0];
                    values[2].TRANSPORTERGROUPNAME = values[1];
                    this.datatable.row($row.get(0)).data(values[2]);

                    var transportergroup = {};
                    if (values[2].SID !== 0) {
                        transportergroup.SID = values[2].SID;
                        transportergroup.IDTRANSPORTERGROUP = values[0];
                        transportergroup.TRANSPORTERGROUPNAME = values[1];
                        $.ajax({
                            type: "POST",
                            url: "/TransporterGroup/Edit",
                            data: '{transportergroup: ' + JSON.stringify(transportergroup) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
                    } else {
                        transportergroup.IDTRANSPORTERGROUP = values[0];
                        transportergroup.TRANSPORTERGROUPNAME = values[1];
                        console.log(transportergroup);
                        $.ajax({
                            type: "POST",
                            url: "../TransporterGroup/Create",
                            data: '{transportergroup: ' + JSON.stringify(transportergroup) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
