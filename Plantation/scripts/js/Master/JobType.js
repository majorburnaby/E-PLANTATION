...
(function (document, window, $) {
    ...
    (function ($) {
        ...
        $('#tbljobtype').on('click', 'tbody td', function (e) {
            if ($(this).index() < 2) {
        ...
        var Table = {
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "SID": 0, "IDJOBTYPE": "", "JOBTYPENAME": "" });
            ...
            rowSave: function ($row) {
                ...
                if (values[0] === "" || values[1] === "") { //Empty Data Validation
                    ...
                    values[2].IDJOBTYPE = values[0];
                    values[2].JOBTYPENAME = values[1];
                    this.datatable.row($row.get(0)).data(values[2]);

                    var jobtype = {};
                    if (values[2].SID !== 0) {
                        jobtype.SID = values[2].SID;
                        jobtype.IDJOBTYPE = values[0];
                        jobtype.JOBTYPENAME = values[1];
                        $.ajax({
                            type: "POST",
                            url: "/JobType/Edit",
                            data: '{jobtype: ' + JSON.stringify(jobtype) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
                    } else {
                    ...
                    $('#tbljobtype').DataTable().ajax.reload();
                }
            },
