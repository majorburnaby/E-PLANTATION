                ...
                $row.children('td').each(function (i) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $this.html('<select id="ddlStockGroup" class="form-control input-sm"></select>');
                            $.getJSON('GetStockGroupList', function (json) {
                                $('#ddlStockGroup').empty();
                                $('#ddlStockGroup').append($('<option>').text("Select"));
                                $.each(json, function (i, obj) {
                                    var selected = (data[4] == obj.SID) ? 'selected' : '';
                                    $('#ddlStockGroup').append($('<option ' + selected + '>').text(obj.STOCKGROUPNAME).attr('value', obj.SID));
                                });
                            });
                        }
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $StockGroupName = $('#ddlStockGroup').find(":selected").text();
                            return $('#ddlStockGroup').find(":selected").val();
                        }
                        if ($(this).parent().children().index($(this)) === 5) {
                ...
                if (values[0]) {
                    stock.SID = values[0];
                    stock.IDSTOCK = values[1];
                    stock.STOCKNAME = values[2];
                    stock.STOCKGROUP = values[3];
                    stock.UOM = values[5];
                    stock.PARTNUMBER = values[7];
                    stock.DESCRIPTION = values[8];
                    stock.REMARK = values[9];
                    stock.ACTIVE = values[10];
                    $.ajax({
                        type: "POST",
                        url: "/Stock/Edit",
