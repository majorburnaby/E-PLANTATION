    ...
    (function ($) {
        ...
        var EditableTable = {
            ...
            build: function () {
                this.datatable = this.$table.DataTable({
                    ...
                    columns: [
                        ...
                        { data: "WORKSHOPGROUP.WORKSHOPGROUPNAME", editField: "WORKSHOP.WORKSHOPGROUP" },
                        {
                    ...
                    "columnDefs": [
                        {
                            ...
                            "targets": 5, "data": "SID", "render": function (data, type, full, meta) {
                                return '<div class="SID" style="display:none;">' + data.WORKSHOP.SID + '</div><a href="#" type="button" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row" data-toggle="tooltip" data-original-title="Edit"><i class="icon md-edit" aria-hidden="true"></i></a>'
                    ...
                    rowCallback: function (row, data) {
                        console.log(data.WORKSHOP.ISACTIVE === 1);
                        // Set the checked state of the checkbox in the table
                        $('input.editor-active', row).prop('checked', data.WORKSHOP.ISACTIVE === 1);
                    }
                });
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "WORKSHOP": { "SID": 0, "IDWORKSHOP": "", "WORKSHOPNAME": "", "WORKSHOPGROUP": "", "ISACTIVE": 0 }, "WORKSHOPGROUP": { "WORKSHOPGROUPNAME": "" } });
            ...
            rowEdit: function ($row) {
                ...
                $row.children('td').each(function (i) {
                    ...
                    if (arrdata.length < 3) {
                        if (i === 0) {
                            content = arrdata[0].IDWORKSHOP;
                        }
                        else if (i === 1) {
                            content = arrdata[0].WORKSHOPNAME;
                        }
                        else if (i === 2) {
                            content = arrdata[0].WORKSHOPGROUP;
                        }
                        else if (i === 3) {
                        ...
                        if (i === 0) {
                            content = arrdata[1].IDWORKSHOP;
                        }
                        else if (i === 1) {
                            content = arrdata[1].WORKSHOPNAME;
                        }
                        else if (i === 2) {
                            content = arrdata[1].WORKSHOPGROUP;
                        }
                        else if (i === 3) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 2) {
                            $this.html('<select id="ddlWorkshopGroup" class="form-control input-sm"></select>');
                            $.getJSON('GetWorkshopGroupList', function (json) {
                                $('#ddlWorkshopGroup').empty();
                                $('#ddlWorkshopGroup').append($('<option>').text("Select").attr('value', ''));
                                $.each(json, function (i, obj) {
                                    var selected = (data.WORKSHOP.WORKSHOPGROUP === obj.SID) ? 'selected' : '';
                                    $('#ddlWorkshopGroup').append($('<option ' + selected + '>').text(obj.WORKSHOPGROUPNAME).attr('value', obj.SID));
                                });
                            });
                        }
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 2) {
                            return $('#ddlWorkshopGroup').find(":selected").val();
                        }
                        if ($(this).parent().children().index($(this)) === 3) {
                ...
                if (values[0] === "" || values[1] === "" || values[2] === "") {
                    ...
                    var workshop = {};
                    if (values[4].WORKSHOP.SID !== 0) {
                        workshop.SID = values[4].WORKSHOP.SID;
                        workshop.IDWORKSHOP = values[0];
                        workshop.WORKSHOPNAME = values[1];
                        workshop.WORKSHOPGROUP = values[2];
                        workshop.ISACTIVE = values[3];
                        $.ajax({
                            type: "POST",
                            url: "/Workshop/Edit",
