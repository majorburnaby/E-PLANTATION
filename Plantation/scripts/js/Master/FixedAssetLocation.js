        ...
        var EditableTable = {
            ...
            build: function () {
                this.datatable = this.$table.DataTable({
                    //dom: "Bfrtip",
                    ajax: "/FixedAssetLocation/Data",
                    columns: [
                        { data: "FIXEDASSETLOCATION.SID" },
                        { data: "FIXEDASSETLOCATION.IDFIXEDASSETLOCATION" },
                        { data: "FIXEDASSETLOCATION.FIXEDASSETLOCATIONNAME" },
                        { data: "FIXEDASSETLOCATION.DESCRIPTION" },
                        { data: "COSTCENTER.COSTCENTERNAME", editField: "FIXEDASSETLOCATION.COSTCENTER" },
                        {
                            data: "FIXEDASSETLOCATION.ISACTIVE",
                    ...
                    "columnDefs": [
                        {
                            ...
                            "targets": 6, "data": "SID", "render": function (data, type, full, meta) {
                                return '<div class="SID" style="display:none;">' + data.FIXEDASSETLOCATION.SID + '</div><a href="#" type="button" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row" data-toggle="tooltip" data-original-title="Edit"><i class="icon md-edit" aria-hidden="true"></i></a>'
                    ...
                    rowCallback: function (row, data) {
                        console.log(data.FIXEDASSETLOCATION.ISACTIVE === 1);
                        // Set the checked state of the checkbox in the table
                        $('input.editor-active', row).prop('checked', data.FIXEDASSETLOCATION.ISACTIVE === 1);
                    }
                });
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "FIXEDASSETLOCATION": { "SID": 0, "IDFIXEDASSETLOCATION": "", "FIXEDASSETLOCATIONNAME": "", "DESCRIPTION": "", "COSTCENTER": "", "ISACTIVE": 0 }, "COSTCENTER": { "COSTCENTERNAME": "" } });
            ...
            rowEdit: function ($row) {
                ...
                $row.children('td').each(function (i) {
                    ...
                    if (arrdata.length < 3) {
                        if (i === 0) {
                            content = arrdata[0].IDFIXEDASSETLOCATION;
                        }
                        else if (i === 1) {
                            content = arrdata[0].FIXEDASSETLOCATIONNAME;
                        }
                        else if (i === 2) {
                            content = arrdata[0].DESCRIPTION;
                        }
                        else if (i === 3) {
                            content = arrdata[0].COSTCENTER;
                        }
                        else if (i === 4) {
                        ...
                        if (i === 0) {
                            content = arrdata[1].IDFIXEDASSETLOCATION;
                        }
                        else if (i === 1) {
                            content = arrdata[1].FIXEDASSETLOCATIONNAME;
                        }
                        else if (i === 2) {
                            content = arrdata[1].DESCRIPTION;
                        }
                        else if (i === 3) {
                            content = arrdata[1].COSTCENTER;
                        }
                        else if (i === 4) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $this.html('<select id="ddlCostCenter" class="form-control input-sm"></select>');
                            $.getJSON('GetCostCenterList', function (json) {
                                $('#ddlCostCenter').empty();
                                $('#ddlCostCenter').append($('<option>').text("Select").attr('value', ''));
                                $.each(json, function (i, obj) {
                                    var selected = (data.FIXEDASSETLOCATION.COSTCENTER === obj.SID) ? 'selected' : '';
                                    $('#ddlCostCenter').append($('<option ' + selected + '>').text(obj.COSTCENTERNAME).attr('value', obj.SID));
                                });
                            });
                        }
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            return $('#ddlCostCenter').find(":selected").val();
                        }
                        if ($(this).parent().children().index($(this)) === 4) {
                ...
                if (values[0] === "" || values[1] === "" || values[2] === "" || values[3] === "") {
                    ...
                    if (values[5].FIXEDASSETLOCATION.SID !== 0) {
                        fixedassetlocation.SID = values[5].FIXEDASSETLOCATION.SID;
                        fixedassetlocation.IDFIXEDASSETLOCATION = values[0];
                        fixedassetlocation.FIXEDASSETLOCATIONNAME = values[1];
                        fixedassetlocation.DESCRIPTION = values[2];
                        fixedassetlocation.COSTCENTER = values[3];
                        fixedassetlocation.ISACTIVE = values[4];
                        $.ajax({
                            type: "POST",
                            url: "/FixedAssetLocation/Edit",
                            data: '{fixedassetlocation: ' + JSON.stringify(fixedassetlocation) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
                    } else {
