        ...
        var Table = {
            ...
            build: function () {
                this.datatable = this.$table.DataTable({
                    //dom: "Blfrtip",
                    ajax: "/SupplierGroup/Data",
                    columns: [
                        { data: "SID" },
                        { data: "IDSUPPLIERGROUP" },
                        { data: "SUPPLIERGROUPNAME" }
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "SID": 0, "IDSUPPLIERGROUP": "", "SUPPLIERGROUPNAME": "" });
            ...
            rowSave: function ($row) {
                ...
                if (values[0] === "" || values[1] === "") { //Empty Data Validation
                    ...
                    values[2].IDSUPPLIERGROUP = values[0];
                    values[2].SUPPLIERGROUPNAME = values[1];
                    this.datatable.row($row.get(0)).data(values[2]);

                    var suppliergroup = {};
                    if (values[2].SID !== 0) {
                        suppliergroup.SID = values[2].SID;
                        suppliergroup.IDSUPPLIERGROUP = values[0];
                        suppliergroup.SUPPLIERGROUPNAME = values[1];
                        $.ajax({
                            type: "POST",
                            url: "/SupplierGroup/Edit",
                            data: '{suppliergroup: ' + JSON.stringify(suppliergroup) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
                    } else {
