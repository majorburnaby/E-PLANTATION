        ...
        var Table = {
            ...
            build: function () {
                this.datatable = this.$table.DataTable({
                    //dom: "Blfrtip",
                    ajax: "/WorkshopGroup/Data",
                    columns: [
                        { data: "SID" },
                        { data: "IDWORKSHOPGROUP" },
                        { data: "WORKSHOPGROUPNAME" }
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "SID": 0, "IDWORKSHOPGROUP": "", "WORKSHOPGROUPNAME": "" });
            ...
            rowSave: function ($row) {
                ...
                if (values[0] === "" || values[1] === "") { //Empty Data Validation
                    ...
                    values[2].IDWORKSHOPGROUP = values[0];
                    values[2].WORKSHOPGROUPNAME = values[1];
                    this.datatable.row($row.get(0)).data(values[2]);

                    var workshopgroup = {};
                    if (values[2].SID !== 0) {
                        workshopgroup.SID = values[2].SID;
                        workshopgroup.IDWORKSHOPGROUP = values[0];
                        workshopgroup.WORKSHOPGROUPNAME = values[1];
                        $.ajax({
                            type: "POST",
                            url: "/WorkshopGroup/Edit",
                            data: '{workshopgroup: ' + JSON.stringify(workshopgroup) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
                    } else {
                    ...
                    $('#tblworkshopgroup').DataTable().ajax.reload();
                }
            },
