                ...
                $row.children('td').each(function (i) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 4) {
                            var checked = (data[5] == 1) ? 'checked' : '';
                            $this.html('<input class="StatusCek" type="checkbox" name="vehicle" value="1" ' + checked + '>');
                        }
                    }
                });
            },
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 4) {
                            if ($('.StatusCek:checked').val()) {
                                return '<input class="StatusCek" type="checkbox" name="vehicle" value="1" checked>';
                            } else {
                                return '<input class="StatusCek" type="checkbox" name="vehicle" value="0">';
                            }
                        }
                ...
                values[5] = $('#StatusCek').val();

                var position = {};
                if (values[0]) {
                    position.SID = values[0];
                    position.IDPOSITION = values[1];
                    position.POSITIONNAME = values[2];
                    position.DESCRIPTION = values[3];
                    position.STATUS = values[4];
                    $.ajax({
                        type: "POST",
                        url: "/Position/Edit",
                    ...
                    position.IDPOSITION = values[1];
                    position.POSITIONTNAME = values[2];
                    position.DESCRIPTION = values[3];
                    position.STATUS = values[4];
                    $.ajax({
                        type: "POST",
