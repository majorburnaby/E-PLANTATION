                ...
                $row.children('td').each(function (i) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $this.html('<select id="ddlFixedAssetGroup" class="form-control input-sm"></select>');
                            $.getJSON('GetFixedAssetGroupList', function (json) {
                                $('#ddlFixedAssetGroup').empty();
                                $('#ddlFixedAssetGroup').append($('<option>').text("Select"));
                                $.each(json, function (i, obj) {
                                    var selected = (data[4] == obj.SID) ? 'selected' : '';
                                    $('#ddlFixedAssetGroup').append($('<option ' + selected + '>').text(obj.FixedAssetGROUPNAME).attr('value', obj.SID));
                                });
                            });
                        }
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $FixedAssetGroupName = $('#ddlFixedAssetGroup').find(":selected").text();
                ...
                var fixedasset = {};
                if (values[0]) {
                    fixedasset.SID = values[0];
                    fixedasset.IDFIXEDASSET = values[1];
                    fixedasset.FIXEDASSETNAME = values[2];
                    fixedasset.FIXEDASSETGROUP = values[3];
                    fixedasset.UOM = values[5];
                    fixedasset.DESCRIPTION = values[7];
                    fixedasset.REMARK = values[8];
                    fixedasset.ACTIVE = values[9];
                    $.ajax({
                        type: "POST",
                        url: "/FixedAsset/Edit",
