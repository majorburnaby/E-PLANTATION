                ...
                $row.children('td').each(function (i) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $this.html('<select id="ddlcontractorGroup" class="form-control input-sm"></select>');
                            $.getJSON('GetcontractorGroupList', function (json) {
                                $('#ddlcontractorGroup').empty();
                                $('#ddlcontractorGroup').append($('<option>').text("Select"));
                                $.each(json, function (i, obj) {
                                    var selected = (data[4] == obj.SID) ? 'selected' : '';
                                    $('#ddlcontractorGroup').append($('<option ' + selected + '>').text(obj.CONTRACTORGROUPNAME).attr('value', obj.SID));
                                });
                            });
                        }
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $contractorGroupName = $('#ddlcontractorGroup').find(":selected").text();
                            return $('#ddlcontractorGroup').find(":selected").val();
                        }
                        if ($(this).parent().children().index($(this)) === 5) {
                ...
                var contractor = {};
                if (values[0]) {
                    contractor.SID = values[0];
                    contractor.IDCONTRACTOR = values[1];
                    contractor.CONTRACTORNAME = values[2];
                    contractor.CONTRACTORGROUP = values[3];
                    contractor.CONTROLJOB = values[5];
                    contractor.CONTACTNAME = values[7];
                    contractor.BANK = values[8];
                    contractor.BANKACCOUNT = values[10];
                    contractor.PHONE = values[11];
                    contractor.FAX = values[12];
                    contractor.EMAIL = values[13];
                    contractor.ADDRESS = values[14];
                    contractor.COUNTRY = values[15];
                    contractor.PROVINCE = values[17];
                    contractor.CITY = values[19];
                    contractor.POSCODE = values[21];
                    contractor.REMARK = values[22];
                    contractor.ACTIVE = values[23];
                    $.ajax({
                        type: "POST",
                        url: "/Contractor/Edit",
                        data: '{contractor: ' + JSON.stringify(contractor) + '}',
                        contentType: "application/json; charset=utf-8",
                        dataType: "json"
                    });
                } else {
