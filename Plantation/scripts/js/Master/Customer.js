                ...
                $row.children('td').each(function (i) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $this.html('<select id="ddlcustomerGroup" class="form-control input-sm"></select>');
                            $.getJSON('GetcustomerGroupList', function (json) {
                                $('#ddlcustomerGroup').empty();
                                $('#ddlcustomerGroup').append($('<option>').text("Select"));
                                $.each(json, function (i, obj) {
                                    var selected = (data[4] == obj.SID) ? 'selected' : '';
                                    $('#ddlcustomerGroup').append($('<option ' + selected + '>').text(obj.customerGROUPNAME).attr('value', obj.SID));
                                });
                            });
                        }
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $customerGroupName = $('#ddlcustomerGroup').find(":selected").text();
                            return $('#ddlcustomerGroup').find(":selected").val();
                        }
                        if ($(this).parent().children().index($(this)) === 5) {
                ...
                var customer = {};
                if (values[0]) {
                    customer.SID = values[0];
                    customer.IDCUSTOMER = values[1];
                    customer.CUSTOMERNAME = values[2];
                    customer.CUSTOMERGROUP = values[3];
                    customer.CONTROLJOB = values[5];
                    customer.CONTACTNAME = values[7];
                    customer.BANK = values[8];
                    customer.BANKACCOUNT = values[10];
                    customer.PHONE = values[11];
                    customer.FAX = values[12];
                    customer.EMAIL = values[13];
                    customer.ADDRESS = values[14];
                    customer.COUNTRY = values[15];
                    customer.PROVINCE = values[17];
                    customer.CITY = values[19];
                    customer.POSCODE = values[21];
                    customer.REMARK = values[22];
                    customer.ACTIVE = values[23];
                    $.ajax({
                        type: "POST",
                        url: "/Customer/Edit",
