        ...
        var Table = {
            ...
            build: function () {
                this.datatable = this.$table.DataTable({
                    //dom: "Blfrtip",
                    ajax: "/BlockOrganization/Data",
                    columns: [
                        { data: "SID" },
                        { data: "IDBLOCKORGANIZATION" },
                        { data: "BLOCKORGANIZATIONNAME" }
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "SID": 0, "IDBLOCKORGANIZATION": "", "BLOCKORGANIZATIONNAME": "" });
            ...
            rowSave: function ($row) {
                ...
                if (values[0] === "" || values[1] === "") { //Empty Data Validation
                    ...
                    values[2].IDBLOCKORGANIZATION = values[0];
                    values[2].BLOCKORGANIZATIONNAME = values[1];
                    this.datatable.row($row.get(0)).data(values[2]);

                    var blockorganization = {};
                    if (values[2].SID !== 0) {
                        blockorganization.SID = values[2].SID;
                        blockorganization.IDBLOCKORGANIZATION = values[0];
                        blockorganization.BLOCKORGANIZATIONNAME = values[1];
                        $.ajax({
                            type: "POST",
                            url: "/BlockOrganization/Edit",
                            data: '{blockorganization: ' + JSON.stringify(blockorganization) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
                    } else {
                        blockorganization.IDBLOCKORGANIZATION = values[0];
                        blockorganization.BLOCKORGANIZATIONNAME = values[1];
                        console.log(blockorganization);
                        $.ajax({
                            type: "POST",
                            url: "../BlockOrganization/Create",
                            data: '{blockorganization: ' + JSON.stringify(blockorganization) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
