...
(function (document, window, $) {
    ...
    // -------------
    var editor;
    var option_allowancedeductiontype = [];
    (function ($) {
        ...
        var Table = {
            ...
            build: function () {
                this.datatable = this.$table.DataTable({
                    //dom: "Bfrtip",
                    ajax: "/AllowanceDeduction/Data",
                    columns: [
                        { data: "ALLOWANCEDEDUCTION.SID" },
                        { data: "ALLOWANCEDEDUCTION.IDALLOWANCEDEDUCTION" },
                        { data: "ALLOWANCEDEDUCTION.ALLOWANCEDEDUCTIONNAME" },
                        { data: "PARAMETERVALUE.PARAMETERVALUENAME", editField: "ALLOWANCEDEDUCTION.ALLOWANCEDEDUCTIONTYPE" }
                    ...
                    "columnDefs": [
                        {
                            ...
                            "targets": 4, "data": "SID", "render": function (data, type, full, meta) {
                                return '<div class="SID" style="display:none;">' + data.ALLOWANCEDEDUCTION.SID + '</div><a href="#" type="button" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row" data-toggle="tooltip" data-original-title="Edit"><i class="icon md-edit" aria-hidden="true"></i></a>'
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "ALLOWANCEDEDUCTION": { "SID": 0, "IDALLOWANCEDEDUCTION": "", "ALLOWANCEDEDUCTIONNAME": "", "ALLOWANCEDEDUCTIONTYPE": "" }, "PARAMETERVALUE": {"PARAMETERVALUENAME":""}});
            ...
            rowEdit: function ($row) {
                ...
                $row.children('td').each(function (i) {
                    ...
                    if (arrdata.length < 3) {
                        if (i === 0) {
                            content = arrdata[0].IDALLOWANCEDEDUCTION;
                        }
                        else if (i === 1) {
                            content = arrdata[0].ALLOWANCEDEDUCTIONNAME;
                        }
                        else if (i === 2) {
                            content = arrdata[0].ALLOWANCEDEDUCTIONTYPE;
                        }
                    }
                    else {
                        if (i === 0) {
                            content = arrdata[1].IDALLOWANCEDEDUCTION;
                        }
                        else if (i === 1) {
                            content = arrdata[1].ALLOWANCEDEDUCTIONNAME;
                        }
                        else if (i === 2) {
                            content = arrdata[1].ALLOWANCEDEDUCTIONTYPE;
                        }
                    }
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 2) {
                            $this.html('<select id="ddlAllowanceDeduction" class="form-control input-sm" data-plugin="select2" style="width: 100%"></select>');
                            $.getJSON('GetAllowanceDeductionList', function (json) {
                                $('#ddlAllowanceDeduction').empty();
                                $('#ddlAllowanceDeduction').append($('<option>').text("Select").attr('value', ''));
                                $.each(json, function (i, obj) {
                                    var selected = (data[4] === obj.SID) ? 'selected' : '';
                                    $('#ddlAllowanceDeduction').append($('<option ' + selected + '>').text(obj.PARAMETERVALUENAME).attr('value', obj.SID));
                                });

                                $('#ddlAllowanceDeduction').val(content);
                            });
                        }
                    }
                    $("#ddlAllowanceDeduction").select2({ allowClear: true });
                });
            },
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 2) {
                            return $('#ddlAllowanceDeduction').find(":selected").val();
                        }
                ...
                if (values[0] === "" || values[1] === "" || values[2] === "") {
                    ...
                    if (values[3].ALLOWANCEDEDUCTION.SID !== 0) {

                        allowancededuction.SID = values[3].ALLOWANCEDEDUCTION.SID;
                        allowancededuction.IDALLOWANCEDEDUCTION = values[0];
                        allowancededuction.ALLOWANCEDEDUCTIONNAME = values[1];
                        allowancededuction.ALLOWANCEDEDUCTIONTYPE = values[2];
                        $.ajax({
                            type: "POST",
                            url: "/AllowanceDeduction/Edit",
                            data: '{allowancededuction: ' + JSON.stringify(allowancededuction) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
                    } else {
