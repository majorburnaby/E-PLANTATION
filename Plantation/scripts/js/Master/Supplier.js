                ...
                $row.children('td').each(function (i) {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $this.html('<select id="ddlSupplierGroup" class="form-control input-sm"></select>');
                            $.getJSON('GetSupplierGroupList', function (json) {
                                $('#ddlSupplierGroup').empty();
                                $('#ddlSupplierGroup').append($('<option>').text("Select"));
                                $.each(json, function (i, obj) {
                                    var selected = (data[4] == obj.SID) ? 'selected' : '';
                                    $('#ddlSupplierGroup').append($('<option ' + selected + '>').text(obj.SUPPLIERGROUPNAME).attr('value', obj.SID));
                                });
                            });
                        }
                        ...
                        if ($(this).parent().children().index($(this)) === 8) {
                            $this.html('<select id="ddlParameterValueBK" class="form-control input-sm"></select>');
                            $.getJSON('GetParameterValueBKList', function (json) {
                                $('#ddlParameterValueBK').empty();
                                $('#ddlParameterValueBK').append($('<option>').text("Select"));
                                $.each(json, function (i, obj) {
                                    var selected = (data[9] == obj.SID) ? 'selected' : '';
                                    $('#ddlParameterValueBK').append($('<option ' + selected + '>').text(obj.PARAMETERVALUENAME).attr('value', obj.SID));
                                });
                            });
                        }

                        if ($(this).parent().children().index($(this)) === 15) {
                            ...
                            $.getJSON('GetCountryList', function (json) {
                                ...
                                $.each(json, function (i, obj) {
                                    var selected = (data[16] == obj.SID) ? 'selected' : '';
                                    $('#ddlCountry').append($('<option ' + selected + '>').text(obj.COUNTRYNAME).attr('value', obj.SID));
                                });
                            });
                        }

                        if ($(this).parent().children().index($(this)) === 17) {
                            ...
                            $.getJSON('GetProvinceList', function (json) {
                                ...
                                $.each(json, function (i, obj) {
                                    var selected = (data[18] == obj.SID) ? 'selected' : '';
                                    $('#ddlProvince').append($('<option ' + selected + '>').text(obj.PROVINCENAME).attr('value', obj.SID));
                                });
                            });
                        }

                        if ($(this).parent().children().index($(this)) === 19) {
                            $this.html('<select id="ddlCity" class="form-control input-sm"></select>');
                            $.getJSON('GetCityList', function (json) {
                                $('#ddlCity').empty();
                                $('#ddlCity').append($('<option>').text("Select"));
                                $.each(json, function (i, obj) {
                                    var selected = (data[20] == obj.SID) ? 'selected' : '';
                                    $('#ddlCity').append($('<option ' + selected + '>').text(obj.CITYNAME).attr('value', obj.SID));
                                });
                            });
                        }
                    }
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 3) {
                            $SupplierGroupName = $('#ddlSupplierGroup').find(":selected").text();
                            return $('#ddlSupplierGroup').find(":selected").val();
                        }
                        if ($(this).parent().children().index($(this)) === 5) {
                        ...
                        if ($(this).parent().children().index($(this)) === 8) {
                            $ParameterValueName = $('#ddlParameterValueBK').find(":selected").text();
                            return $('#ddlParameterValueBK').find(":selected").val();
                        }
                        if ($(this).parent().children().index($(this)) === 15) {
                        ...
                        if ($(this).parent().children().index($(this)) === 17) {
                            $ProvinceName = $('#ddlProvince').find(":selected").text();
                            return $('#ddlProvince').find(":selected").val();
                        }
                        if ($(this).parent().children().index($(this)) === 19) {
                            $CityName = $('#ddlCity').find(":selected").text();
                            return $('#ddlCity').find(":selected").val();
                        }
                ...
                var supplier = {};
                if (values[0]) {
                    supplier.SID = values[0];
                    supplier.IDSUPPLIER = values[1];
                    supplier.SUPPLIERNAME = values[2];
                    supplier.SUPPLIERGROUP = values[3];
                    supplier.CONTROLJOB = values[5];
                    supplier.CONTACTNAME = values[7];
                    supplier.BANK = values[8];
                    supplier.BANKACCOUNT = values[10];
                    supplier.PHONE = values[11];
                    supplier.FAX = values[12];
                    supplier.EMAIL = values[13];
                    supplier.ADDRESS = values[14];
                    supplier.COUNTRY = values[15];
                    supplier.PROVINCE = values[17];
                    supplier.CITY = values[19];
                    supplier.POSCODE = values[21];
                    supplier.REMARK = values[22];
                    supplier.ACTIVE = values[23];
                    $.ajax({
                        type: "POST",
                        url: "/Supplier/Edit",
