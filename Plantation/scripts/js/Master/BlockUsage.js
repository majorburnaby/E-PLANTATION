        ...
        var Table = {
            ...
            build: function () {
                this.datatable = this.$table.DataTable({
                    //dom: "Blfrtip",
                    ajax: "/BlockUsage/Data",
                    columns: [
                        { data: "BLOCKUSAGE.SID" },
                        { data: "PARAMETERVALUE.PARAMETERVALUENAME" },
                        { data: "BLOCKUSAGE.HECTARAGE" }
                    ...
                    "columnDefs": [
                        ...
                        , {
                            ...
                            "targets": 3, "data": "SID", "render": function (data, type, full, meta) {
                                return '<div class="SID" style="display:none;">' + data.BLOCKUSAGE.SID + '</div><a href="#" type="button" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row" data-toggle="tooltip" data-original-title="Edit"><i class="icon md-edit" aria-hidden="true"></i></a>'
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "BLOCKUSAGE": { "SID": 0, "BLOCKMASTER": $('#SID').val(), "USAGE": 0, "HECTARAGE": 0 }, "PARAMETERVALUE": { "PARAMETERVALUENAME": "" } });
            ...
            rowEdit: function ($row) {
                ...
                $row.children('td').each(function (i) {
                    ...
                    if (arrdata.length < 3) {
                        if (i === 0) {
                            content = arrdata[0].USAGE;
                        }
                        else if (i === 1) {
                            content = arrdata[0].HECTARAGE;
                        }
                    }
                    else {
                        if (i === 0) {
                            content = arrdata[1].USAGE;
                        }
                        else if (i === 1) {
                            content = arrdata[1].HECTARAGE;
                        }
                    }
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 0) {
                            $this.html('<select id="ddlUsage" class="form-control input-sm"></select>');
                            var SIDBLOCKMASTER = $('#SID').val();

                            //new data
                            if (arrdata.length < 3) {
                                $.getJSON('../../BlockUsage/GetUnusedUsageList', { BlockMaster: SIDBLOCKMASTER }, function (json) {
                                    $('#ddlUsage').empty();
                                    $('#ddlUsage').append($('<option>').text("Select").attr('value', '0'));
                                    $.each(json, function (i, obj) {
                                        var selected = (data.BLOCKUSAGE.USAGE == obj.SID) ? 'selected' : '';
                                        $('#ddlUsage').append($('<option ' + selected + '>').text(obj.PARAMETERVALUENAME).attr('value', obj.SID));
                                    });

                                });

                            } else {
                                $.getJSON('../../BlockUsage/GetUsageList', function (json) {
                                    $('#ddlUsage').empty();
                                    $('#ddlUsage').append($('<option>').text("Select").attr('value', '0'));
                                    $.each(json, function (i, obj) {
                                        var selected = (data.BLOCKUSAGE.USAGE == obj.SID) ? 'selected' : '';
            ...
            rowSave: function ($row) {
                ...
                values = $row.find('td').map(function () {
                    ...
                    if ($this.hasClass('actions')) {
                        ...
                        if ($(this).parent().children().index($(this)) === 0) {
                            return $('#ddlUsage').find(":selected").val();
                        }
                ...
                if (values[0] == '0' || values[1] == '' || values[1] == '0') { //Empty Data Validation
                    bootbox.dialog({
