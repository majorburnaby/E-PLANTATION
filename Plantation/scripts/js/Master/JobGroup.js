...
(function (document, window, $) {
    ...
    (function ($) {
        ...
        $('#tbljobgroup').on('click', 'tbody td', function (e) {
            if ($(this).index() < 2) {
        ...
        var Table = {
            ...
            rowAdd: function () {
                ...
                data = this.datatable.row.add({ "SID": 0, "IDJOBGROUP": "", "JOBGROUPNAME": "" });
            ...
            rowSave: function ($row) {
                ...
                if (values[0] === "" || values[1] === "") { //Empty Data Validation
                    ...
                    values[2].IDJOBGROUP = values[0];
                    values[2].JOBGROUPNAME = values[1];
                    this.datatable.row($row.get(0)).data(values[2]);

                    var jobgroup = {};
                    if (values[2].SID !== 0) {
                        jobgroup.SID = values[2].SID;
                        jobgroup.IDJOBGROUP = values[0];
                        jobgroup.JOBGROUPNAME = values[1];
                        $.ajax({
                            type: "POST",
                            url: "/JobGroup/Edit",
                            data: '{jobgroup: ' + JSON.stringify(jobgroup) + '}',
                            contentType: "application/json; charset=utf-8",
                            dataType: "json"
                        });
                    } else {
                    ...
                    $('#tbljobgroup').DataTable().ajax.reload();
                }
            },
